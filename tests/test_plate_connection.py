"""Validation for the energy-conserving bridge to a **grid plate** body (Step 4).

The string terminus drives a distributed simply-supported Kirchhoff plate (model #5) at a single
driving-point node through a linear spring. It faces the same battery as the modal bridge: the total
energy ``E_string + E_plate + E_conn`` is conserved to machine precision (the string's own energy is
*not*, once coupled), passivity when either part is lossy, a bit-identity to the uncoupled parts at
``K = 0``, a real energy-transfer check (the plate carries a substantial share), the radiation
read-out carrying the coupling force, and the exact Sherman-Morrison stability guard rejecting an
over-stiff spring. The plate is the first *distributed* (non-modal) body, so the coupling injects a
point force into the plate's implicit theta-scheme RHS rather than a modal generalized force.
"""

import numpy as np
import pytest
from helpers import make_plate_bridge
...
def _ceiling_K(**kw) -> float:
    ...
    probe = make_plate_bridge(K=1000.0, **kw)
...
def test_total_energy_conserved_across_lambda(lam):
    bridge = _pluck(make_plate_bridge(lam=lam))
...
def test_total_energy_conserved_across_stiffness(K):
    bridge = _pluck(make_plate_bridge(lam=0.9, K=K))
...
def test_string_energy_alone_is_not_conserved():
    ...
    bridge = _pluck(make_plate_bridge(lam=0.9))
    s = bridge.string
    e_str = [s.energy()]
...
# -- energy transfer: the plate ends up carrying a substantial share --------------------------
def test_energy_flows_string_to_plate():
    ...
    assert max_plate_frac > 0.1, f"plate only ever held {max_plate_frac:.1%} of the energy"


# -- Criterion (passivity): with loss anywhere, the total decreases monotonically ------------
def test_passivity_with_plate_damping():
    bridge = _pluck(make_plate_bridge(lam=0.9, sigma_plate=10.0))
    res = simulate(bridge, num_steps=int(1.5 * bridge.string.fs))
    e0 = res.energy[0]
...
def test_passivity_with_string_damping():
    bridge = _pluck(make_plate_bridge(lam=0.9, sigma_string=5.0))
    res = simulate(bridge, num_steps=int(1.5 * bridge.string.fs))
    e0 = res.energy[0]
    steps_up = np.diff(res.energy)
    assert np.all(steps_up <= 1e-12 * e0), f"max positive step {steps_up.max():.2e}"
...
def test_K0_bit_identical_to_uncoupled_parts():
    bridge = make_plate_bridge(K=0.0)
    s, p = bridge.string, bridge.plate
    s.set_state(triangular_pluck(s.x, s.L, 0.137 * s.L, amplitude=1e-3))
    # excite the plate too, so its path is non-trivial
    u0 = 1e-3 * np.sin(np.pi * p.X / p.Lx) * np.sin(np.pi * p.Y / p.Ly)
    p.set_state(u0)
    ...
    p_ref = Plate(Lx=p.Lx, Ly=p.Ly, kappa=p.kappa, rho=p.rho, fs=p.fs, N=p.N)
    p_ref.set_state(u0)
    ...
    assert np.array_equal(p.u, p_ref.u), "plate not bit-identical to uncoupled at K=0"
...
def test_pressure_includes_coupling_term():
    ...
    # driving force. Compare against an identical plate stepped from the same state with NO force:
    # the difference must equal A^-1 s_F (the force distributed through the implicit operator), the
    # exact coupling term the naive bending reconstruction -kappa^2 B u would drop.
    bridge = _pluck(make_plate_bridge(K=6000.0))
    p = bridge.plate
    ...
    p_free = Plate(Lx=p.Lx, Ly=p.Ly, kappa=p.kappa, rho=p.rho, fs=p.fs, N=p.N)
    ...
    s_f[bridge.drive_index] = F / (p.rho * p.h * p.h)
    ...
    assert np.allclose(p._accel - p_free._accel, expected_diff, rtol=0, atol=1e-9), \
        "plate acceleration does not carry the injected coupling force"

    naive_pressure = p.h * p.h * float(np.sum(p_free._accel))  # what dropping the force gives
...
def test_unstable_stiffness_rejected():
    with pytest.raises(ValueError, match="unstable"):
        make_plate_bridge(lam=0.9, K=1e7)  # far past the guard bound
...
def test_margin_is_linear_in_stiffness():
    ...
    m1 = make_plate_bridge(K=1000.0).stability_margin
    m3 = make_plate_bridge(K=3000.0).stability_margin
...
def test_guard_holds_at_its_boundary():
    ...
    bridge = _pluck(make_plate_bridge(K=K_in))
...
def test_just_over_the_ceiling_is_rejected():
    Kc = _ceiling_K()
    make_plate_bridge(K=0.95 * Kc)  # just inside: constructs fine
    with pytest.raises(ValueError, match="unstable"):
        make_plate_bridge(K=1.05 * Kc)  # just outside: rejected


def test_ceiling_is_the_true_instability_onset():
    # The guard's ceiling must be the *actual* physical onset, not merely self-consistent (a ceiling
    # inflated a few % by a rank-1 algebra slip would sail past the 0.93x-conserves check and NaN a
    # user mid-render -- the modal-bridge footgun, one class down). Bracket the physical onset: just
    # inside conserves; just past it (beta_s and f_ext are K-independent, so bumping .K after
    # construction is a clean guard bypass) actually diverges. This is the verification the sibling
    # StringBodyBridge earns by assembling the real coupled operator.
    Kc = _ceiling_K()
    inside = _pluck(make_plate_bridge(K=0.99 * Kc))
    ...
    over = _pluck(make_plate_bridge(K=0.5 * Kc))
...
def test_right_end_must_be_free():
    ...
    p = Plate(Lx=1.0, Ly=1.0, kappa=20.0, rho=0.005, fs=fs, N=16)
    with pytest.raises(ValueError, match="free"):
        StringPlateBridge(string=s, plate=p, K=1000.0)


def test_plate_must_be_supported():
    fs = 22000.0
