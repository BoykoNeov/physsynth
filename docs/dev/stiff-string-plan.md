...
> **Status: IMPLEMENTED (2026-06-21).** `core/string_stiff.py` (implicit θ-scheme), oracles in
> `analysis/modal.py` + `analysis/dispersion.py`, suite in `tests/test_stiff_string.py`, viz in
> `viz/plots.py::plot_stiff_partials` + `scripts/diagnose_stiff_string.py`. All acceptance tests
> green. Build followed the de-risk order below (κ=0 machinery → biharmonic → stretched partials).
> Mirrors `ideal-string-plan.md`. Tolerances inherit Milestone 1's bar (human's call, 2026-06-21).
>
> **Key build decisions taken (vs. the plan as written):**
> - Biharmonic block built as `D2 @ D2` (squared Dirichlet 2nd-difference), *not* hand-coded "5"
>   rows. This makes the energy SBP identity `⟨δ_xxxx f,g⟩ = ⟨δ_xx f, δ_xx g⟩` exact to machine
>   precision and produces the correct `5/h⁴` boundary row automatically — see `operators.py`.
> - Energy `P(f,g)` is evaluated through the *same* matrix `𝓛` used in the update, so conservation
>   is an exact algebraic identity (drift ~1e-12 observed, well under the 1e-10 bar).
> - θ default = **0.28** (a hair above ¼, accuracy-first with a small positivity margin).
> - See "Damping caveat" below — a scheme property discovered during implementation.
...
### Damping caveat — the θ-scheme makes frequency-*independent* loss effectively frequency-dependent

Discovered during implementation (worth pinning so it is not later mistaken for a bug). The discrete
per-step decay factor for mode *m* is

```
ρ² = (1 − σk + θ·Q·k²) / (1 + σk + θ·Q·k²)   ⇒   E_m(t) ~ exp(−2σ·(1 − θ·Q·k²)·t)
```

So a mode decays at `2σ·(1 − θQk²)`, **not** `2σ`. For low modes `Qk² ≪ 1` and the rate is `2σ` to a
fraction of a percent (this is what `test_decay_rate_matches_2sigma_low_mode` checks). But `Q = c²p² +
κ²p⁴` is huge for high modes — with stiffness the `κ²p⁴` term dominates — so `θQk²` reaches O(10) for
the top partials at a musical grid. Those modes **under-damp drastically** (a broadband pluck at κ=2,
N=256 retained ~100× the analytic energy after 1 s: ~57% off `e^{−2σt}`, not "a few %"). Passivity
still holds unconditionally (`ρ² ≤ 1` for any σ ≥ 0 — energy only ever decreases); it is the *rate*,
not the sign, that is wrong at high frequency.

Audibly this is **backwards** from a real string (where high partials die *faster*). It is a genuine
property of frequency-independent loss discretized with a θ-time-average at finite `k`, not an
implementation error. The cure is **model #3 (frequency-dependent loss)**, which sets the per-mode
decay deliberately rather than inheriting this artifact. Until then, the σ-path's broadband decay
fidelity is a documented limitation; the lossless physics (energy, partials, dispersion) is unaffected.
