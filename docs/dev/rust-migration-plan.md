...
### 44.9 The freeze's own bug: an exact comparison arrived by DEFAULT where the project had decided against one

Caught in review, after the batch was green and pushed, and it is the sharpest thing in it.

`flatten` collects every integer in a returned value and the test compares them **exactly**. That
is right for a mode label, a multiplicity, an FFT length and a root count. `RotatingWave` is a
`NamedTuple` whose fields include `iterations` — how many Newton steps the continuation took — and
ledger #33 settled that the iteration count is **not a comparable quantity**: when only the root
survives, two implementations may take visibly different paths to the same answer, and 17 of 108
fixtures did. `tests/test_rust_parity_rotating_wave.py` carried a test whose entire job was to say
so, `test_the_iteration_count_is_not_compared_and_this_is_the_witness`, "kept as an executable
statement of what this file declines to assert".

**That file was deleted in this batch and the freeze reinstated the assertion its witness existed
to forbid** — silently, and green, because neither rotating-wave fixture happens to be one of the
ones that differ. The witness fixture that does (`n = 48, mode = 1, kappa = 2.0`) is not in the
case table.

The general shape is the finding, and it is specific to *freezing* rather than to porting: **a
recording mechanism applies one rule to everything it records.** A hand-written comparison makes a
decision per quantity — this one exactly, that one to a tolerance, this one not at all — and every
one of those decisions is an argument somebody had. A generic flattener has no way to know which
integers were arguments. So the sweep is not optional: for every integer and boolean in the record,
ask whether it is **structure** (a length, a label, a count — where exact is right and is often the
sharpest thing in the file) or a **decision** (an iteration count, a predicate — which needs either
a recorded margin or an exclusion).

Both came out of that sweep here:

* `iterations` is blanked in `analysis_frozen_cases.call` — in `call` rather than in the test, so
  the generator and the test see the same object and a future regeneration cannot reintroduce it.
* `damping.discrete_damped_mode_is_underdamped` is a **decision**, and its margin was *reported*
  rather than asserted by the parity test that has just been retired. That claim is now
  `test_the_underdamped_predicate_has_room_to_spare` — five configurations, modes 1 to 40, the
  discriminant never within eleven orders of zero relative to its own scale — and it carries a
  second assertion the original did not need: that the sweep still answers **both** `True` and
  `False`. Which is how the other half of this was found. The `#over` fixture was returning `True`
  like its sibling, so the pair covered one branch twice and the case that exists to pin the
  overdamped arm pinned nothing.

Regenerated by restoring the six Python bodies from `git show b1d37c8~1:...`, re-running the
generator, and restoring the shims with an explicit `git checkout HEAD -- physsynth/analysis/`
(explicit path, per the scar about `git checkout --` taking HEAD over uncommitted work). **Three
lines of `analysis_frozen_values.py` moved and nothing else did** — which is the point of
regenerating rather than hand-editing a file whose own header says it is generated: the diff is the
proof that the restore-and-delete was clean.

### 44.10 What the next batch inherits
