...
| 69 | **When a seam audit's gate has already been discharged, the audit becomes a coverage question.** §35.5 defined the viewer audit as the gate on deletions: every name `web/serialize.py` reaches past a public constructor had to exist on the Rust object before the body behind it could go. With all eleven units deleted that half is discharged **by construction** — a missing name is an `AttributeError` in the viewer *today*, not a future hazard — so what was left is the half no grep can answer: **a reach only proves the binding exposes it if the line executes.** Run as line coverage over `web/serialize.py` (94%, 408 tests) intersected with a reach set derived over the package rather than a name list (#67): 635 candidate lines from 408 attribute names on 23 core modules, **three dark** — two defensive branches provably unreachable from every call site, one a name collision on the serializer's own record. The fourth, found the same way, was **live**: a bow render whose animation window covers the whole audio run collapses the settle window to nothing and captures `bow.state` at step 0, a path the suite had never run. Three corollaries. **Grep counts prose** — three of thirteen private "reaches" were a comment and two docstrings, so the surface is measured with `ast.walk`, which cannot see a comment. **A read audit passes a file that cannot write** — §33's read-only-getter scar means the six state *writes* onto the geometric string are their own row. And **reachability analysis is what separates a dark line needing a test from one needing nothing**: two of the three dark lines are guarded by `min`/`max` expressions that make their branch unreachable, and a test for either would assert behaviour no caller can request | §50.1, §50.2, §50.5, §50.6 |
