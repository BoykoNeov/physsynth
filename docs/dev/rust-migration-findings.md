...
> **What this is.** The seventy-eight findings the migration has made about when two
...
| 60 | **A *split* deletion empties the guard that would have covered it.** `deleted_bodies` asserts three things at once — the module reads no flag, defines no `<Name>Py` alias, and each named object *is* the Rust one — and a module deleted in halves satisfies only the third, so the entry cannot be added and the identity claim goes unasserted for the whole interval between the two commits. Every physics test still passes, because there is only one implementation left for them to run. Cure: a second table asserting the third alone **plus** that the module still reads the flag, so it cannot be left behind when the halves rejoin | §47.3 |
| 61 | **Widening a draining population does not fix it if the wider population is *mixed*.** The extension-import scan was moved off `test_rust_parity*.py` onto "every test file mentioning the extension" and its docstring called that "a population that grows"; the parity files mention it too, so the set loses one per deletion and gains one per binding-surface test. It failed at 7 of a floor of 10 when one deletion took four files. #41's *other* cure is the durable one: name a **positive control** that is permanent by construction and assert it is found, rather than counting. A count was never the subject | §47.4 |
| 62 | **A parity family does not partition along the tiers it tests**, because a parity file deliberately builds a *mixed* configuration: the airbox wrapper file holds a port alias so it can put a Rust wrapper over a **Python** port (§28.4), so deleting either tier retires all four files and the other half of the split retires none. Corollary for scoping a split: check which aliases each test file names before assuming the tests divide the way the classes do — and note that the mixture a parity file exercises is usually the *opposite* of the one a split leaves running | §47.1, §47.2 |
| 63 | **A `module.X is physsynth_rs.X` guard cannot cover residue that is not a Rust object.** Every deletion leaves §41.2's three categories behind and `deleted_bodies` covers re-exports because a re-export *is* a Rust object. `connection.py`'s shim keeps `sparse`, `spsolve` and `splu` for one reason only — `connection.rs` reads all three off the module namespace at call time — so the loop iterates straight past them and their sole protection is `noqa: F401`, one `ruff check --fix` from an `AttributeError` raised inside the extension three layers from the cause. The airbox seams (§48.1) were under the same `noqa` and *were* covered, by accident of being Rust classes; nothing about the two shims' shape shows the difference. Cure: assert the identity against the library **and** monkeypatch the names to prove the binding reaches them — presence and reachability are two claims | §49.2 |
| 64 | **Killing the twin can make a comparison test STRONGER, so read a parity file test by test before deleting it.** "Rust agrees with Python about `np.sum`'s pairwise blocking" only ever asserted that two transcriptions coincided — the Python could have been wrong about NumPy and the test would have passed. Re-aimed at the real referent it becomes `rs.beta_b == rs.k**2 * float(np.sum(terms))`, which cannot. Same for the reduction that reaches the next timestep (drive two **bare** bodies alongside the coupler with the two spellings; one must track, one must not — the negative control is what keeps the first from being vacuous) and for a decision *not* to transcribe: the binding looks `dot` up on the `numpy` module at call time, so replacing the name **counts the calls** and pins §14.2's choice directly instead of observing it through a trajectory. 26 of `test_rust_parity_connection.py`'s 36 tests outlived the twin; only the four trajectories and the two cross-language anchors died, and both anchors were already asserted within Rust | §49.3 |
| 65 | **A pinned call count can be the operator's dimension.** Building a `StringBodyBridge` makes exactly `N + M` `np.dot` calls, because the exact stability guard assembles the coupled leapfrog operator column by column and every column's `eta` needs the same product (52 at `N=48, M=4`); `energy` makes exactly one; `step` makes **none**, because the reference takes the body's own `bridge_displacement`. Three numbers, each a statement about the scheme rather than about a value — and all three are invisible to every physics bar. Corollary of #64: when a comparison dies, look for a *count* or a *shape* to assert in its place | §49.3 |
| 68 | **A freeze promotes a comparison's incidental digits into an assertion's load-bearing ones.** The parity test asserted a *substring of the prose* (`"connection unstable" in str(err)`) and then compared the two languages' full messages **on one machine**; writing the message down as a literal when the twin dies silently upgrades every digit to a cross-machine claim — #14 through a door nobody had used, and unrepeatable locally (there is no repro on Windows). Two of `connection`'s eleven refusals interpolate `np.linalg.eigvals` on a 52x52 dense matrix and `spsolve` + `splu(...).solve(...)`, whose ordering is a claim about how SciPy was *built*. Cure: freeze the prose exactly, mark the number with a placeholder, and assert only that it is still reported and is a finite positive float. **Before freezing a string, ask which of its characters the old test actually compared, and on how many machines** | §49.3 |
| 66 | **`__all__` in a deleted module is load-bearing, not documentation.** A shim's re-exports are unused imports to ruff, so without `__all__` marking them used, `ruff check --fix` deletes the module's entire public surface. `airbox.py` has been relying on this since §47 without saying so and `connection.py` had never had one, so the last deletion had to add it. The second `ruff --fix` hazard of the deletion phase, and it fails in the same direction as the first (#63): green until something outside the file asks for a name | §49.4 |
| 67 | **An expectation that reaches zero can be re-derived over a WIDER population instead of deleted.** `expected_classes` held four rows and emptying it gives `set() == set()` plus two loops over an empty dict — #52's mechanism through a fourth door, and #60's cure (delete the table) was the right one there. Here the *derive* was worth more at zero than at four: the hand-written tuple it read became every core module the test imports, and the claim became "no module in `physsynth.core` chooses between two implementations of a class any more", which is live over twenty-one modules and is what would fail if a reference implementation came back. Three findings in this file say a derive is only as wide as its tuple (§17.6, `collision` for a batch, `radiation` never in the table) — and **widening the tuple by hand does not fix that, it restates it at a larger number**: the first draft listed twenty-one modules and a twenty-fourth added tomorrow would sit outside it exactly as `collision` did. The population has to be *derived* too, here `pkgutil.iter_modules(physsynth.core.__path__)` (23 modules, `engine` and `portable` included, neither ever in the tuple), with named positive controls rather than a floor proving the scan fires (#61). So the question at zero is not "delete or empty" but **"is there a wider population this derive was always a special case of, and can I compute it?"** | §49.4 |
| 69 | **When a seam audit's gate has already been discharged, the audit becomes a coverage question.** §35.5 defined the viewer audit as the gate on deletions: every name `web/serialize.py` reaches past a public constructor had to exist on the Rust object before the body behind it could go. With all eleven units deleted that half is discharged **by construction** — a missing name is an `AttributeError` in the viewer *today*, not a future hazard — so what was left is the half no grep can answer: **a reach only proves the binding exposes it if the line executes.** Run as line coverage over `web/serialize.py` (94%, 409 tests) intersected with a reach set derived over the packages rather than a name list (#67): 680 candidate lines from 633 attribute names on 29 modules, **three dark** — two defensive branches provably unreachable from every call site, one a name collision on the serializer's own record. The fourth, found the same way, was **live**: a bow render whose animation window covers the whole audio run collapses the settle window to nothing and captures `bow.state` at step 0, a path the suite had never run. Three corollaries. **Grep counts prose** — three of thirteen private "reaches" were a comment and two docstrings, so the surface is measured with `ast.walk`, which cannot see a comment. **A read audit passes a file that cannot write** — §33's read-only-getter scar means the six state *writes* onto the geometric string are their own row. **A derive is only as wide as the population it walks even when the population is a package** — the first version of this very scan took `physsynth.core` only, and only class `dir()`, so every reach onto an `analysis` oracle's result and every module-level name was outside it; widening moved the numbers and not the conclusion, but the narrow claim was the one written down. And **reachability analysis is what separates a dark line needing a test from one needing nothing**: two of the three dark lines are guarded by `min`/`max` expressions that make their branch unreachable, and a test for either would assert behaviour no caller can request | §50.1, §50.2, §50.5, §50.6 |
| 70 | **A constraint that came from a DELETION does not transfer to a MOVE, and "there is no second implementation" is the one to check.** `docs/dev/resolution-horizon-plan.md` §6 refused to promote seven test-folder helpers into `physsynth/analysis/` because a new public name trips `test_analysis_frozen.py`'s derived guard and "satisfying it is impossible — there is no Python implementation left to freeze against", so the only route was a guard amendment paid for by one function. Sound reasoning, false premise: that fact belongs to §44's *deleted* modules, whose bodies are gone forever. A **promotion** has a live Python body on one side of it right up to the commit that replaces it, so the freeze window is one commit wide and walking through it (`scripts/freeze_horizon.py`, twelve cases, Python and Rust measured in the same pass) leaves the contract over the other 62 fixtures untouched. Same shape as #67 one level up: the honest question at an apparent dead end is *whose* constraint this is. Two corollaries. The **sequencing is the only unrecoverable part** — record and commit before the edit that removes the second implementation, and make the generator refuse loudly afterwards rather than emit a half-file. And a **dependency worry stated about a function name is not a dependency worry**: §6 also called `scipy.optimize.brentq` "a dependency decision in the analysis crate, whose allowlist is deliberately narrow", when that crate already `#[path]`-includes a transcription of SciPy's own `brentq.c` for `modal.rs` — the allowlist stayed empty and `Cargo.toml` did not move. Ask what the crate already *compiles*, not what the call is *named* | horizon plan §11.1, §11.2 |
| 71 | **A hand-written inventory of "what has no home outside the binding" measures the hand, not the crate — derive it.** The retirement plan's §11 listed 14 classes across 2 binding files, found by reading the two files a batch happened to touch. Mechanically: every `#[pyclass(name = ...)]` in `physsynth-py/src` (43) minus every `pub struct`/`pub enum` in `physsynth-core/src` (88), hand-clearing case-spelling false positives (`VKPlate` vs core's `VkPlate`, `MalletVKPlate` vs `MalletVkPlate` — a Python-facing name capitalising an acronym the Rust name does not), gives **16 across 3**. The missed file, `airbox_port.rs`, is the tier the other two stand on: its `Patch` holds room, `T`, `R` and `load_matrix` as `Py<PyAny>` while the core module of the same name exports free functions only, so every wrapper above calls `require_ready`/`free_pressure`/`inject`/`reset` on a class with no native half. The same understatement one level down: `core::AirBox` was a *shell* whose `step` never called the port kernels that sat beside it in the crate, because only the binding — which does not hold a `core::AirBox` at all — ever reached them. **The porting order is not a preference, it is read off which slot takes which type**: ports, then wrappers, then the bridges whose `body=` and `plate=` slots take the wrappers | retirement plan §13.1 |
| 72 | **A last-bit choice that no native bar can see is not a defect, but it is unrecoverable once the reference goes — so the cross-implementation check belongs in the batch that ports the class.** The lumped port's `u_free = np.dot(a, q^{n+1} - q^{n-1}) / 2k` is the one `np.dot` in `physsynth-core` that reaches the timestep (the crate's three other `dot` copies each say in their doc comment that they are read-outs and spell themselves `s += x * y`). Spelled that way it diverges from BLAS at the **second step** by one ulp, and 200 steps later 661 of 693 room pressure nodes differ at 1.9e-12 — the room amplifying one ulp with the body's own state still bit-identical, i.e. #6's "does the reduction reach the next timestep" answered *yes* through a collaborator rather than through the model. `ddot` **fuses** its multiply-add; `x.mul_add(y, s)` reproduces it exactly, measured at two mode counts. And restoring the plain loop leaves all 26 native bars green: both spellings are physically correct, so nothing but the retiring reference could ever have chosen between them. Same for the injection book's association (`total + (t0 + t1)` versus `(total + t0) + t1`), which only two injections in one step can distinguish — a scene the crate could not even express before this batch | retirement plan §13.3 |
| 73 | **A new bar written to catch a known trap must be run against that trap, because the fixture can neuter it.** `the_two_ledgers_agree_across_the_terminal` was added precisely because the conserved total is structurally blind to a wrong `R_room` — and with both instruments at *interior* nodes it passed with the `1 / (1 + beta)` wall-closure factor deleted, since `beta` is zero inside the room and the factor is exactly 1. One instrument moved to a corner and it goes red. The mutation is also what turns the retired file's header claim into a measurement: with the factor gone, all six conservation and passivity bars stay green and only the three differential ones fail. **Mutate the thing the bar is for; a bar that has never been red is a claim about a fixture** | retirement plan §13.6 |
| 74 | **Reproducing a Python back-reference in Rust is a choice with a blast radius, and the constraint that decides it is a test.** `RoomPort` reaches into its room to queue an injection and the room reaches back into every port to unstick it — a cycle Rust will not have. `test_two_heads_share_one_room` (two instruments, one room) rules out the wrapper owning the room, leaving *the port is a value and the room is passed at each call*, and the two invariants the reference kept on the room each cost one field: `claims`, which is `room._ports` reduced to what the disjointness message actually reads, and `epoch`, bumped by `set_state`, which replaces writing `_queued_at = -1` into ports the room no longer holds. The epoch is **not** a restatement of the step count and the retired test cannot tell: there the port queued at step 0 and `set_state` returns the room to step 0, so a bare step comparison refuses and only the epoch saves it — which is why the substitution needs a bar of its own naming the mechanism | retirement plan §13.2 |
| 75 | **An operand-order note is load-bearing only when the constant is not a power of two.** The suspended room closure carried a careful transcription comment pinning `(2 R) q` over `2 (R q)`, and a mutation swapping them changes **no digit anywhere** — not one of 49 plate nodes, 1560 room nodes or 200 ledger values, over four scenes. Multiplying a double by 2 is exact and binary rounding is scale-invariant, so `round(2 r q) = 2 round(r q)` short of overflow or subnormals. The other two mutations in the same batch *were* seen instantly (re-associating the two room load terms reddens everything; reversing the interior port's pressure jump reddens exactly the two suspended scenes), so the check discriminates and this one is genuinely vacuous. Two corollaries: a pin costs nothing to keep but should not be *counted* as a verified constraint, and the real hazard is the same note written about a constant that is **not** a power of two — `(0.5 k) / rho` against `0.5 (k / rho)` pins a real bit because the division does not commute with the scaling | retirement plan §16.4 |
| 76 | **`np.allclose(a, b, atol=X)` keeps `rtol=1e-5`, so a translated tolerance is almost never the one written down.** A retired bar read `atol=1e-18` and looks like a claim at 1e-18 absolute; what NumPy actually ran was `|a - b| <= 1e-18 + 1e-5 |b|`, seven orders of magnitude looser on a quantity of order 1e-3. Carried over as the `atol` alone the native bar **failed**, and correctly — the load matrix's triple product leaves a node and its 180° image 2e-15 apart *relatively*, which the reference had never asserted anything about. The cure is not to relax to the reference's bar but to read what the reference was entitled to claim and then choose: the native version keeps the relative form at 1e-12, seven decades tighter than what shipped. **Before carrying any `allclose`, write out the inequality it evaluated, including the default it did not name** — and the same question applies to `pytest.approx` (relative 1e-6 by default) and to `assert_allclose` (whose default `rtol` is 1e-7, not 1e-5) | retirement plan §16.5 |
| 77 | **A piece of what is left to port can be a *mode* of a class, and no audit that counts classes will list it.** Batch 5 of the retirement finished the room wrappers and checked the one client that drives them. `MalletVKPlate` is native, but handed a room wrapper instead of a bare plate it switches to a second step (`step_in_room`) that composes the wrapper's `prepare` / `loaded_rhs` / `finish` around its own outer iteration, and that composition exists **only in the binding**. It is invisible to a hand count and to §13.1's derive alike, because both enumerate *classes*, and this is a branch inside one whose class name is already on the native side. The detector is a grep for every site that **recognises** the collaborator types (`VkRoom::of`, casts to the wrapper classes, `in_room`) rather than for the classes themselves: here it found exactly one, and that is the difference between a hole of six and a hole of six plus one. **Before declaring a tier finished, grep for who branches on its types, not only for who defines them** | retirement plan §17.5 |
| 78 | **A bar that asserts two answers DIFFER cannot see a half-routed override when the other half already makes them differ — write a positive oracle.** The room gong's exact tangent has two overrides, the influence column and the operator it is inverted against. The retired Python bar compared the room's tangent with a bare gong's and asserted they are not equal; the retargeted column alone guarantees that, so the bar was green with the operator left on the plate's own factorization — the exact defect the mallet-room batch had recorded as its scar — and a faithful native carry was green under the same mutation. The cure is a claim with a referent of its own: the tangent is `-d w_node / d f` at the root, so a central difference of the room's own trial solve must match it (routed 2.6e-12 - 9.5e-11, unrouted 9.1e-6, bar 1e-8). The same batch found §17.1's hand-written commit a second time, in the native bare mallet, which a bar seeded with garbage now pins. **A difference from a twin that differs in two ways certifies neither; mutate each half separately (#73)** | retirement plan §18.3, §18.4 |
| 79 | **A defensive line that guarded against a caller's move the native type forbids is unobservable — measure it, then delete it rather than carry it.** The binding's plate bridge zeroed its force vector's drive entry after every step, because there a caller could replace `_f_ext` or move `drive_index` between steps. Natively neither can change after construction, so the only nonzero entry is always the one about to be overwritten: the mutation that dropped the reset failed **no** bar and changed **no** bit, and the shape bar's own comment claiming it would catch a leak was false. The general form: when a port turns an attribute into a private field or a construction-time value (retirement plan §14.2: a refusal about shape becomes a type), every line that defended against writes to it becomes dead, and a mutation is how to find which. The same batch sharpened an equality bar into a positive one (#78's shape again): "the gong's margin equals the areal-density twin's" could not rule out a margin that ignores density altogether, so the twin built with the volume density must now come out different (0.1394 vs 0.1425) | retirement plan §20.3 |
...
13. If this test's twin were deleted tomorrow, what would it still be asserting? A comparison that has a *referent* outside the twin -- NumPy's blocking, a call count, a buffer's identity -- survives, and usually gets sharper. (#64, #65)
14. What inequality did the reference's tolerance *evaluate*? `np.allclose` keeps `rtol=1e-5` when you pass only `atol`, so a carried bar is routinely orders of magnitude away from the claim it looks like. (#76)
15. Is anything left that is not a class? A client can switch to a different step when it is handed one of the types you just ported, and that branch lives wherever the client does. Grep for every site that *recognises* the ported types. (#77)
16. Does the bar assert a *difference* from a twin? Count the ways the twin differs. If the override under test is one of two, the other can carry the bar alone — replace it with a positive oracle that has its own referent. (#78)
17. Is a line defensive against a write the native type no longer allows? Delete it and run the bars: if nothing fails, it was guarding a door the port bricked up — remove it, and fix any comment that claimed a bar would see it. (#79)
