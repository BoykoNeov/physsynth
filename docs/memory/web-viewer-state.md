...
b9/b10, excited strings → b2/b8/b11/b14, weinreich → b7, parametric demo → b16). So there is **no
batch 17 to look up**, and a future session must not infer one from the document: a new viewer batch
needs a new *model* or a new *capability* to exist first. The built-vs-shown gap that Phase D existed
